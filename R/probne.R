...
#' @param strategy_weight Numeric in [0,1]. Strategy weight.
...
fuzzy_vikor <- function(decision_matrix, criteria_direction, strategy_weight = 0.5,
  ...
  if (!is.numeric(strategy_weight) || length(strategy_weight) != 1 || strategy_weight < 0 || strategy_weight > 1) {
  ...
  Compromise <- strategy_weight * (TotalLoss - TotalLoss_best) / (TotalLoss_worst - TotalLoss_best) +
    (1 - strategy_weight) * (MaxLoss - MaxLoss_best) / (MaxLoss_worst - MaxLoss_best)
